...
router.get("/",async(req:Request,res:Response)=>{
    try{
        ...
        if(deviceId){
            ...
            let freeDevices = await passwordModel.find({isfree:true});
                if(freeDevices.length > 0){
                    return res.json({ success: true, freeDevice: freeDevices });
                }else{
                    return res.json({ success: false, message:"No free devices available"});
                }
        ...
        return res.json({success:false,message:"Error in getting Free Placement"})
    }
})
